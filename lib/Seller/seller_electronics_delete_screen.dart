...
//
// class ElectronicsEditScreen extends StatefulWidget {
//   final String userId;
//
//   ElectronicsEditScreen({required this.userId});
//
//   @override
//   _ElectronicsEditScreenState createState() => _ElectronicsEditScreenState();
// }
//
// class _ElectronicsEditScreenState extends State<ElectronicsEditScreen> {
...
//     return Scaffold(
//       appBar: AppBar(
//         title: Text('Electronics Edit Screen'),
//       ),
//       body: _isLoading
//           ? Center(child: CircularProgressIndicator())
//           : _footwears.isEmpty
//           ? Center(child: Text('No Electronics found.'))
//           : ListView.builder(
...
class MobilesEditScreen extends StatefulWidget {
  final String userId;

  const MobilesEditScreen({super.key, required this.userId});

  @override
  _MobilesEditScreenState createState() => _MobilesEditScreenState();
}

class _MobilesEditScreenState extends State<MobilesEditScreen> {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  List<Map<String, dynamic>> _footwears = [];
  bool _isLoading = true;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Mobiles Edit Screen'),
      ),
      body: _isLoading
...
class _EditClothingScreenState extends State<EditClothingScreen> {
  ...
  Future<String> _uploadImage(File imageFile, String clothingId, String colorIndex, String imageIndex) async {
    try {
      final Reference storageReference = FirebaseStorage.instance
          .ref()
          .child('mobiles_images/$clothingId/$colorIndex/$imageIndex.jpg');
  ...
  Future<String> _uploadVideo(File videoFile, String clothingId, String colorIndex, String videoIndex) async {
    try {
      final Reference storageReference = FirebaseStorage.instance
          .ref()
          .child('mobiles_videos/$clothingId/$colorIndex/$videoIndex.mp4');
  ...
  Future<void> _updateClothing() async {
    if (_formKey.currentState!.validate()) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Mobiles updated successfully')),
        );
