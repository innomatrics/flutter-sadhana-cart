...
class ClothingsDeleteScreen extends StatefulWidget {
  ...
  const ClothingsDeleteScreen({super.key, required this.userId});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Clothings Edit Screen'),
      ),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator())
          : _footwears.isEmpty
          ? const Center(child: Text('No clothings found.'))
          : ListView.builder(
        itemCount: _footwears.length,
        itemBuilder: (context, index) {
          var footwear = _footwears[index];
  ...
  Future<String> _uploadImage(File imageFile, String clothingId, String colorIndex, String imageIndex) async {
    try {
      final Reference storageReference = FirebaseStorage.instance
          .ref()
          .child('clothing_images/$clothingId/$colorIndex/$imageIndex.jpg');
...
Future<String> _uploadVideo(File videoFile, String clothingId, String colorIndex, String videoIndex) async {
  try {
    final Reference storageReference = FirebaseStorage.instance
        .ref()
        .child('clothing_videos/$clothingId/$colorIndex/$videoIndex.mp4');
